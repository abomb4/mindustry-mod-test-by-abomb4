...
const fan = extendContent(PowerTurret, 'meltdown-fan', {
    load() {
        this.super$load();
        this.baseRegion = Core.atlas.find(lib.aModName + "-meltdown-fan-base");
    },
    generateIcons(){
        const list = this.super$generateIcons();
        list[0] = Core.atlas.find(lib.aModName + "-meltdown-fan-base");
